...
O projeto foi desenvolvido para a disciplina de Performance em Sistemas Ciberfísicos, com foco em explorar conceitos de FreeRTOS, multitarefa entre os dois núcleos do ESP32, sincronização com mutexes, persistência local de dados e profiling de desempenho de um sistema embarcado real.
...
Core 1 — TaskSensores
Responsável exclusivamente pela leitura dos sensores, em ciclo configurável:

leitura dos sensores IR E18-D80NK, com uma máquina de estados que identifica a sequência de acionamento (A→B ou B→A) para determinar o sentido da passagem;
leitura do encoder KY-040 via interrupção de hardware, contando passos e convertendo-os em uma passagem quando um número mínimo de pulsos é atingido;
medição do tempo de execução de cada rotina e cálculo do uso de CPU do núcleo.
Core 0 — loop() principal
Responsável pela rede e pela interface:

servidor web (WebServer) que entrega as páginas estáticas (armazenadas em LittleFS) e expõe uma API própria em JSON;
sincronização periódica de horário via NTP, com re-tentativa automática enquanto a rede não está disponível;
atualização do display OLED;
salvamento periódico (a cada 30s) dos dados, gráficos e logs em LittleFS.
